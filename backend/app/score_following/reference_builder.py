...
class ReferenceBuilder:
    ...
    def __init__(
        ...
        self.base_bpm: float | None = None  # score's written tempo (for absolute BPM)
    ...
    def build(self) -> np.ndarray:
        ...
        # Remember the written tempo so the follower can report an absolute BPM.
        tempo_map = self._build_tempo_map(score)
        self.base_bpm = float(tempo_map[0][1]) if tempo_map else None
    ...
    # Harmonic template: (semitone offset above the fundamental, relative weight).
    # Mirrors the first partials of a real (piano) tone — fundamental, octave,
    # octave+fifth, two octaves, +major third, +fifth.  Adding these makes the
    # synthetic reference chroma resemble the chroma of *recorded* audio, which
    # is what the follower actually receives, dramatically improving matching.
    _HARMONICS: List[Tuple[int, float]] = [
        (0, 1.0), (12, 0.5), (19, 0.33), (24, 0.25), (28, 0.20), (31, 0.16),
    ]

    def _build_piano_roll(self, events: List[NoteEvent], n_frames: int) -> np.ndarray:
        """Build a piano-roll matrix (n_frames × 128) with harmonic overtones."""
        ...
        for midi, onset, dur, _, _ in events:
            ...
            for semis, weight in self._HARMONICS:
                p = midi + semis
                if 0 <= p < 128:
                    roll[f_on:f_off, p] += weight
        return roll
