...
class OnlineDTW:
    ...
    def __init__(
        ...
        stay_penalty: float = 0.20,    # cost added to "waiting" (anti-lag)
    ) -> None:
        ...
        self._conf_history: Deque[float] = deque(maxlen=smooth_k)
    ...
    def step(self, observation: np.ndarray) -> tuple[int, float]:
        ...
        for i in range(n_lo, n_hi + 1):
            ...
            # Three predecessors (Dixon 2005, eq. 1).  The horizontal step (the
            # reference frame stays put while the observation advances) is what
            # makes the follower fall behind on slow / self-similar music, so it
            # carries a small penalty: waiting must cost a little more than making
            # diagonal progress, which keeps the follower pacing the performer.
            preds = (
                self._prev[i] + self.stay_penalty,               # horizontal (wait)
                (self._prev[i - 1] if i > 0 else np.inf),        # diagonal (advance)
                (cur[i - 1]        if i > 0 else np.inf),        # vertical (skip)
            )
            best_pred = min(preds)
        ...
        # The accumulated cost is computed over the full ±W band above (for DP
        # robustness), but the *position* is read from a forward-only sub-window
        # capped at max_speed_ratio frames.  This does two things a plain
        # band-wide argmin can't:
        #   • Forward-only by construction — the score never rewinds, so we never
        #     get pinned to the start (a plain argmin ties toward the lowest
        #     index and stalls when the player plays faster than the reference).
        #   • Bounded advance — ambiguous input (silence, noise) can move the
        #     position at most max_speed_ratio frames per hop instead of jumping.
        # Among near-optimal cells we pick the most-advanced one so tempo
        # increases are tracked instead of lagging behind.
        # Read the position from a forward-only sub-window of the accumulated
        # cost, capped at max_speed_ratio frames.  Forward-only means the score
        # never rewinds; the cap stops ambiguous input from jumping.  Among
        # near-optimal cells pick the most advanced so tempo increases are
        # tracked rather than lagged.  The stay-penalty baked into the DP above
        # is what actually keeps this from standing still.
        max_adv = max(1, int(round(self.max_speed_ratio)))
        fwd_lo = self._n
        fwd_hi = min(self._n + max_adv, self.N - 1)
        fwd_slice = cur[fwd_lo:fwd_hi + 1]

        min_cost = float(np.min(fwd_slice))
        tol = 1e-4 * (1.0 + abs(min_cost))
        near = np.nonzero(fwd_slice <= min_cost + tol)[0]
        best_offset = int(near.max())
        raw_pos = fwd_lo + best_offset
        ...
        # ── Confidence ────────────────────────────────────────────────────────
        # Based on how well *this* observation matches the chosen reference frame
        # (local cosine similarity), not on the accumulated DTW cost.  The
        # accumulated cost only ever grows, so the old ratio-of-running-max
        # formula collapsed to ~0 within seconds and told the user nothing.
        # A short median over recent frames keeps the value stable.
        local_dist = self._cosine_dist(self._ref[raw_pos], obs)
        self._conf_history.append(max(0.0, 1.0 - local_dist))
        confidence = round(float(np.median(list(self._conf_history))), 4)
